...
public class CraftItemFlag {
   ...
   private static final Collection<DataComponentType<?>> HIDE_ADDITIONAL_TOOLTIP;
   ...
   public static Collection<DataComponentType<?>> bukkitToNMS(ItemFlag bukkit) {
      if (bukkit == ItemFlag.HIDE_ADDITIONAL_TOOLTIP) {
         ...
         DataComponentType<?> type = (DataComponentType)BUKKIT_TO_NMS.get(bukkit);
         if (type == null) {
            NamespacedKey key = bukkit.getComponent();
            Preconditions.checkArgument(key != null, "Unknown flag %s must have component key", bukkit);
            type = (DataComponentType)BuiltInRegistries.DATA_COMPONENT_TYPE.getValue(CraftNamespacedKey.toMinecraft(key));
         }
   ...
   public static ItemFlag nmsToBukkit(DataComponentType<?> nms) {
      ...
      if (inverse == null) {
         ...
         String var10000 = key.getPath().toUpperCase(Locale.ROOT);
         inverse = ItemFlag.valueOf("HIDE_" + var10000.replace('/', '_'));
      }
   ...
   static {
      HIDE_ADDITIONAL_TOOLTIP = Arrays.asList(DataComponents.POTION_CONTENTS, DataComponents.WRITABLE_BOOK_CONTENT, DataComponents.WRITTEN_BOOK_CONTENT, DataComponents.FIREWORKS, DataComponents.FIREWORK_EXPLOSION, DataComponents.MAP_COLOR, DataComponents.MAP_DECORATIONS, DataComponents.MAP_ID, DataComponents.MAP_POST_PROCESSING, DataComponents.BANNER_PATTERNS, DataComponents.STORED_ENCHANTMENTS);
