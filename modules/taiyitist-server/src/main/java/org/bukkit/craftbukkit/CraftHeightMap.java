...
public final class CraftHeightMap {
   ...
   public static Heightmap.Types toNMS(HeightMap bukkitHeightMap) {
      switch (bukkitHeightMap) {
         case MOTION_BLOCKING_NO_LEAVES -> {
            return Types.MOTION_BLOCKING_NO_LEAVES;
         }
         case OCEAN_FLOOR -> {
         ...
         case WORLD_SURFACE -> {
            return Types.WORLD_SURFACE;
         }
         case WORLD_SURFACE_WG -> {
            return Types.WORLD_SURFACE_WG;
         }
         case MOTION_BLOCKING -> {
            return Types.MOTION_BLOCKING;
         }
         default -> throw new EnumConstantNotPresentException(Heightmap.Types.class, bukkitHeightMap.name());
      }
   }
   ...
   public static HeightMap fromNMS(Heightmap.Types nmsHeightMapType) {
      switch (nmsHeightMapType) {
         ...
         default -> throw new EnumConstantNotPresentException(HeightMap.class, nmsHeightMapType.name());
      }
   }
}
