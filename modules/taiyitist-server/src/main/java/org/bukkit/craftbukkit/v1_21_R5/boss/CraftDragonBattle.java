...
public class CraftDragonBattle implements DragonBattle {
   ...
   public boolean initiateRespawn(Collection<EnderCrystal> list) {
      if (this.hasBeenPreviouslyKilled() && this.getRespawnPhase() == RespawnPhase.NONE) {
         ...
         Collection<EnderCrystal> list = list != null ? new ArrayList(list) : Collections.emptyList();
         ((Collection)list).removeIf((enderCrystal) -> {
         ...
         return this.handle.respawnDragon((List)((Collection)list).stream().map((enderCrystal) -> {
            return ((CraftEnderCrystal)enderCrystal).getHandle();
