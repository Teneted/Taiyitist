...
public class CraftLimitedRegion extends CraftRegionAccessor implements LimitedRegion {
      ...
      if (!this.entitiesLoaded) {
         ...
         for(int x = -1; x <= 1; ++x) {
            for(int z = -1; z <= 1; ++z) {
               ...
               Iterator var5 = chunk.getEntities().iterator();
   ...
   public void saveEntities() {
      ...
      var5 = this.outsideEntities.iterator();
   ...
   public List<BlockState> getTileEntities() {
      ...
      for(int x = -1; x <= 1; ++x) {
         for(int z = -1; z <= 1; ++z) {
            ...
            Iterator var5 = chunk.getBlockEntitiesPos().iterator();

            while(var5.hasNext()) {
               BlockPos position = (BlockPos)var5.next();
   ...
   public void addEntityToWorld(Entity entity, CreatureSpawnEvent.SpawnReason reason) {
      this.entities.add(entity);
   }

   public void addEntityWithPassengers(Entity entity, CreatureSpawnEvent.SpawnReason reason) {
      this.entities.add(entity);
   }
}
