...
import org.teneted.taiyitist.bukkit.remapping.generated.TaiyitistReflectionHandler;
...
public class TaiyitistRedirectAdapter implements PluginTransformer {
    ...
    public static void scanMethod(byte[] bytes) {
        ...
        TaiyitistMCStart.LOGGER.debug(MARKER, "Scanning {}", reader.getClassName());
                    ...
                    if (METHOD_MODIFY.containsKey(key) || METHOD_REDIRECT.containsKey(key)) {
                        try {
                            ...
                            if (target != null) {
                                ...
                                if (bridge != null) {
                                    TaiyitistMCStart.LOGGER.debug(MARKER, "Creating bridge handler {}/{}{} to {}", node.name, method.name, method.desc, methodToString(target));
