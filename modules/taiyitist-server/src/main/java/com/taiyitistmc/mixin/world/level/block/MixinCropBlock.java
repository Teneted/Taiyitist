...
public abstract class MixinCropBlock {
    ...
    private boolean taiyitist$callEntityChangeBlockEvent(boolean original, @Local(argsOnly = true) BlockPos blockPos, @Local ServerLevel serverLevel, @Local(argsOnly = true) Entity entity) {
        return CraftEventFactory.callEntityChangeBlockEvent(entity, blockPos, Blocks.AIR.defaultBlockState(), !serverLevel.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING));
    }
}
